...
with torch.no_grad():
  ...
  for i in npydata:
    ...
    lidar_static    = from_polar_np(np.load(args.data + "lidar/s{}.npy".format(str(i)))[:,:,::int(64/args.beam),::args.dim].astype('float32'))
    lidar_dynamic   = from_polar_np(np.load(args.data + "lidar/d{}.npy".format(str(i)))[:,:,::int(64/args.beam),::args.dim].astype('float32'))
    # lidar_static = lidar_dynamic
    # out = np.ndarray(shape=(lidar_dynamic.shape[0],3,args.beam,dim2))
    # print(lidar_static.shape, lidar_dynamic.shape)  #correct

    
    lidar_dynamic = lidar_dynamic.reshape(-1, 3, args.beam *  dim2)
    # lidar_static = lidar_static.reshape(-1, 3, args.beam *  dim2)
    ...
    for batch in loader:
        ...
        recon = model( lidar_d )
        # recon     = F.softmax(recon)
        # lidarStat = F.softmax(lidar_s)
        recon     = (recon.reshape(-1,3,args.beam, dim2))
        lidarStat = (lidar_s)
        # print(recon.shape, lidar_s.shape)
        # exit(0)
        
        # losses += [directed_hausdorff(recon.reshape(-1,3, args.beam * int(1024/args.dim)) , lidar_s.reshape(-1,3, args.beam * int(1024/args.dim)) )]
        # losses += [ (criterion(recon.reshape(-1,3, args.beam * int(1024/args.dim)) , lidar_s.reshape(-1,3, args.beam * int(1024/args.dim)) )) ]
        # losses1 += [ torch.sqrt(criterion(recon.reshape(-1,3, args.beam * int(outer/args.dim)) , lidar_s.reshape(-1,3, args.beam * int(outer/args.dim)) )) ]
        # 
        # print(torch.stack(losses).mean().item())
        # losses1 += [directed_hausdorff(recon.reshape(-1,3,16 * dim2, (lidar_static).reshape(-1,3,16 * dim2)))]
        # lidar_s = lidar_s.reshape(-1,3, args.beam, dim2)
        # recon = recon.reshape(-1,3, args.beam, dim2)

        # losses1 += [ jsd( lidar_s , recon ) ]
        losses2 += [ calc_mmd_loss( lidar_s , recon ) ]
        # losses += torch.Tensor([emd_samples(recon.detach().cpu().numpy(), lidar_s.detach().cpu().numpy(), bins = 40)])
        # losses += [loss_fn(recon , lidar_s)]
    ...
    # print('RMSE  {}: {:.8f}'.format(i, losses1))
    print('MMD {}: {:.8f}'.format(i, losses2))
    # print('CD {}: {:.8f}'.format(i, losses))
    # print('EMD {}: {:.8f}'.format(i, losses1))
    print('------------------')
...
# print('JSD ', jsdmet/len(npydata))
# print('RMSE ', rmse/len(npydata))
# print(emd/len(npydata))
# print(cd/len(npydata))
print('MMD ', mmd/len(npydata))