...
with torch.no_grad():
  ...
  for i in npydata:
    ...
    for batch in loader:
        ...
        #EMD
        losses1 += [ torch.sqrt(criterion(recon , lidar_s)) ]
        losses2 += torch.Tensor([emd_samples(recon.detach().cpu().numpy(), lidar_s.detach().cpu().numpy(), bins = 40)])

        losses3 += [loss_fn(recon , lidar_s)]  #CD
    ...
    print('EMD {}: {:.8f}'.format(i, losses2))
    print('CD {}: {:.8f}'.format(i, losses3))
    print('------------------')
    rmse += losses1
    emd  += losses2
    cd   += losses3
...
print('EMD ',emd/len(npydata))
print('CD ',cd/len(npydata))
