...
class netG(nn.Module):  #decoder
    def __init__(self, args, nz=100, ngf=64, nc=3, base=4, ff=(1,2)):
        ...
        layers += [nn.ConvTranspose2d(ngf * 4, ngf * 4, (3,4), stride=2, padding=(1,1), bias=False)]
        layers += [nn.BatchNorm2d(ngf * 4)]
        layers += [nn.ReLU(True)]
        ...
        layers += [nn.ConvTranspose2d(ngf, nc, (2,4), 2, 1, bias=False)]
        layers += [nn.Tanh()]
...
class netD(nn.Module): #encoder
    def __init__(self, args, ndf=64, nc=2, nz=1, lf=(1,2)):
        ...
        layers += [nn.LeakyReLU(0.2, inplace=True)]
        layers += [nn.Conv2d(ndf*2, ndf * 2, (2,4), 2, 1, bias=False)]
        ...
        layers += [nn.LeakyReLU(0.2, inplace=True)]
        layers += [nn.Conv2d(ndf * 4, ndf * 8, (1,3), 2, (0,1), bias=False)]
