...
with torch.no_grad():
  ...
  for i in npydata:
    ...
    for batch in loader:
        ...
        lidar_s = lidar_s.reshape(-1,3, args.beam * int(1024/args.dim))
        ...
        losses2 += [ calc_mmd_loss(((recon.reshape(-1,3, args.beam* int(outer/args.dim)))) , (lidar_s.reshape(-1,3, args.beam* int(outer/args.dim)) )) ]
        
        # losses2 += torch.Tensor([emd_samples(recon.detach().cpu().numpy(), lidar_s.detach().cpu().numpy(), bins = 40)])
        # losses3 += [loss_fn(recon.reshape(-1,3, args.beam, int(1024/args.dim)) , lidar_s.reshape(-1,3, args.beam, int(1024/args.dim)))]  #CD  
