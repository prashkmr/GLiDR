...
def train(args, io):
    ...
    if args.model == 'pointnet':
        model = PointNet(args).cuda()
    elif args.model == 'dgcnn':
        model = DGCNN_Topo(args).cuda()
    else:
        raise Exception("Not implemented")
    print(str(model))
    print('Summary')
    # model = nn.DataParallel(model)
    summary(model, (3,args.beam * int(512/args.dim)))
    ...
    if args.reload != '':
        net = torch.load(args.reload)
        model.load_state_dict(net['state_dict'])
        opt.load_state_dict(net['optimizer'])
        print('Reloaded')
    ...
    for epoch in range(0, 100):
        ...
        for dataloader in train_loader:
            ...
            for data in dataloader:
                ...
                dynamic_data = data[1].cuda()
                static_data = data[2].cuda()
