...
def load_dytost(dynamic, static,args):
    ...
    for i, j in zip(dynamic, static):
        ...
        # exists(0)
        dynamic_lidar = dynamic_lidar[:,:,::int(16/args.beam),::args.dim].astype('float32') # 2048 [, 2, 40, 256]     this has 2048    samples
        print("Dynamic", dynamic_lidar.shape)
        dynamic_lidar = from_polar_np(dynamic_lidar).reshape(-1, 3, args.beam * int(1024/args.dim)) # 2048, 3, 40, 256 = > 20480, 3, 1024
        print(dynamic_lidar.shape)

        static_lidar = static_lidar[:,:,::int(16/args.beam),::args.dim].astype('float32') # 2048 [, 2, 40, 256]     this has 2048    samples
        print("Static", static_lidar.shape)
        static_lidar = from_polar_np(static_lidar).reshape(-1, 3, args.beam * int(1024/args.dim) ) # 2048, 3, 40, 256 = > 20480, 3, 1024 
        print(static_lidar.shape) 
